...
def plot_comparative_roc():
    print("--- Generating Comparative ROC Curves on Statlog ---")
    ...
    for (name, file), color in zip(models_to_evaluate.items(), colors):
        ...
        # Plot Line
        # Make the ensemble line thicker to stand out
    ...
    plt.title('Cross-Domain Generalization: ROC Curves on Statlog', fontsize=14)
    plt.legend(loc="lower right", fontsize=11)
    plt.grid(alpha=0.3)
    
    save_path = os.path.join(results_dir, 'comparative_roc_statlog.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"SUCCESS: Comparative ROC Curve saved to {save_path}")
